...
import { ResourceAllocationApprovalTaskModal } from './ResourceAllocationApprovalTaskModal'
...
function ApprovalStepResolver({ approvalStepId, onClose, onSuccess, onError, children }: ApprovalStepResolverProps) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      else { setError('Could not resolve allocation from approval step.'); onError?.('Could not resolve allocation from approval step.') }
  ...
  if (loading) { return (<Box sx={{ p: 4, textAlign: 'center' }}><CircularProgress size={32} /><Typography variant="body2" color="text.secondary" sx={{ mt: 2 }}>Resolving resource allocation...</Typography></Box>) }
  if (error || !entityId) { return (<Box sx={{ p: 3 }}><Alert severity="error" sx={{ borderRadius: 1.5 }}>{error || 'Unable to resolve allocation for this task.'}</Alert></Box>) }
  return <>{children(entityId)}</>
}

export interface ResourceAllocationApprovalTaskModalWrapperProps {
  approvalStepId: string
  onClose: () => void
  onSuccess?: (msg: string) => void
  onError?: (msg: string) => void
...
export const ResourceAllocationApprovalTaskModalWrapper: React.FC<ResourceAllocationApprovalTaskModalWrapperProps> = ({
  ...
  return (
    <ApprovalStepResolver approvalStepId={approvalStepId} onClose={onClose} onSuccess={onSuccess} onError={onError}>
      {(allocationId) => (
        <ResourceAllocationApprovalTaskModal
          open={open}
          onClose={handleClose}
