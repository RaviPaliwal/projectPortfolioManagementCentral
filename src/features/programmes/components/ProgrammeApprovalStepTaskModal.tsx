...
import { ProgrammeApprovalTaskModal } from './ProgrammeApprovalTaskModal'
...
function ApprovalStepResolver({ approvalStepId, onClose, onSuccess, onError, children }: ApprovalStepResolverProps) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      else { setError('Could not resolve programme from approval step.'); onError?.('Could not resolve programme from approval step.') }
  ...
  if (loading) { return (<Box sx={{ p: 4, textAlign: 'center' }}><CircularProgress size={32} /><Typography variant="body2" color="text.secondary" sx={{ mt: 2 }}>Resolving programme...</Typography></Box>) }
  if (error || !entityId) { return (<Box sx={{ p: 3 }}><Alert severity="error" sx={{ borderRadius: 1.5 }}>{error || 'Unable to resolve programme for this task.'}</Alert></Box>) }
  return <>{children(entityId)}</>
}

export interface ProgrammeApprovalTaskModalWrapperProps {
  approvalStepId: string
  onClose: () => void
  onSuccess?: (msg: string) => void
  onError?: (msg: string) => void
...
export const ProgrammeApprovalTaskModalWrapper: React.FC<ProgrammeApprovalTaskModalWrapperProps> = ({
