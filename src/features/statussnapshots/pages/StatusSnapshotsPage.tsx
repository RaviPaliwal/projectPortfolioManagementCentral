...
import { PageHeader, KpiCardRow, TableFooter, TableShell, SearchFilterBar, DetailDrawer, TabPanel, ExportButton } from '@/components/common'
...
function RagChip({ value, label }: { value?: number | string | null; label?: string }) {
  ...
  const color = v === 1 ? '#22c55e' : v === 0 ? '#f59e0b' : v === 2 ? '#ef4444' : '#94a3b8'
  const bgcolor = isDark
    ? (v === 1 ? '#052e16' : v === 0 ? '#1c1917' : v === 2 ? '#450a0a' : '#1e293b')
    : (v === 1 ? '#f0fdf4' : v === 0 ? '#fffbeb' : v === 2 ? '#fef2f2' : '#f8fafc')
  const IconComponent = v === 1 ? CheckCircleIcon : v === 0 ? WarningAmberIcon : v === 2 ? ErrorIcon : ChecklistIcon
  return (
    <Chip
      icon={<IconComponent sx={{ fontSize: 14, color: color }} />}
      label={label || 'Not Set'}
      size="small"
      sx={{
        fontWeight: 700, borderRadius: 8, minWidth: 72,
    ...
    <Box>
      ...
      <Box sx={{ display: 'flex', gap: 1, mb: 2, flexWrap: 'wrap' }}>
        <ExportButton
          ...
          filename={'StatusSnapshots_' + new Date().toISOString().slice(0, 10)}
        />
      </Box>
                  ...
                  <Card variant="outlined" sx={{
                    ...
                    <CardContent sx={{ p: 3, '&:last-child': { pb: 3 } }}>
                      ...
                      <Stack direction="row" spacing={1} useFlexGap sx={{ flexWrap: 'wrap' }}>
                        <Chip icon={<CheckCircleIcon sx={{ fontSize: 14 }} />} label={dist.green + ' Green'} size="small" sx={{ fontWeight: 600, borderRadius: 8, bgcolor: isDark ? '#052e16' : '#f0fdf4', color: '#22c55e' }} />
                        <Chip icon={<WarningAmberIcon sx={{ fontSize: 14 }} />} label={dist.amber + ' Amber'} size="small" sx={{ fontWeight: 600, borderRadius: 8, bgcolor: isDark ? '#1c1917' : '#fffbeb', color: '#f59e0b' }} />
                        <Chip icon={<ErrorIcon sx={{ fontSize: 14 }} />} label={dist.red + ' Red'} size="small" sx={{ fontWeight: 600, borderRadius: 8, bgcolor: isDark ? '#450a0a' : '#fef2f2', color: '#ef4444' }} />
                        {dist.notset > 0 && (
                          <Chip label={dist.notset + ' Not Set'} size="small" sx={{ fontWeight: 600, borderRadius: 8 }} />
                        )}
                      </Stack>
                    </CardContent>
                  </Card>
              ...
              <TableBody>
                {paginatedSnapshots.map((snapshot, idx) => (
                  ...
                  >
                    ...
                    <TableCell>
                      <Chip label={snapshot.pm_entitytype || '\u2014'} size="small" variant="outlined" sx={{ fontWeight: 600, borderRadius: 8 }} />
                    </TableCell>
                    <TableCell>
      ...
      <DetailDrawer
        ...
        subtitle={selectedSnapshot && (
          <Chip label={selectedSnapshot.pm_entitytype || '\u2014'} size="small" variant="outlined" sx={{ fontWeight: 600, borderRadius: 8 }} />
        )}
        headerActions={
            ...
            <Paper variant="outlined" sx={{ p: 2.5, borderRadius: 2 }}>
              ...
              <Box sx={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 2 }}>
                ...
                <Box>
                  ...
                  <Chip label={Number(selectedSnapshot.pm_approvalstatus) === 0 ? 'Approved' : 'Pending'} color={Number(selectedSnapshot.pm_approvalstatus) === 0 ? 'success' : 'warning'} size="small" sx={{ fontWeight: 600, borderRadius: 8 }} />
                </Box>
              </Box>
            </Paper>
