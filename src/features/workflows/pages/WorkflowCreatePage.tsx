...
import { createWorkflow, createWorkflowStepTemplate, fetchOwnerTeams } from '@/services'
...
const MODULES = [
  ...
  {value: 'GateReview', label: 'Gate Reviews' },
...
export default function WorkflowCreatePage({ onStepChange, onCreated }: Props) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      // Save Steps
      const stepPromises = stepTemplates.map((step, idx) => {
        return createWorkflowStepTemplate({
          ...step,
          pm_steporder: idx + 1,
          _pm_workflowlookup_value: result.pm_workflowid,
        } as any)
      })
      ...
      setError('Failed to create workflow and steps. Please try again.')
    } finally {
      setSaving(false)
    }
  }
  ...
  const openAddStep = () => {
    ...
    setStepFormData({
      ...
      pm_assigneeid: '', pm_description: '', pm_sladays: 5,
    })
  ...
  const saveStep = () => {
    ...
    if (editingStepIdx !== null) {
      newSteps[editingStepIdx] = { ...stepFormData }
    } else {
    ...
    <Box sx={{ pt: 2 }}>
          ...
          <Box sx={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
            <Stack spacing={1}>
              ...
              <Typography variant="body2" color="text.secondary">
                Provide a unique name and select the module this workflow will govern.
              </Typography>
            </Stack>
            ...
            <Stack spacing={3}>
              <TextField
                ...
                placeholder="e.g. Project Approval Process"
              ...
              <TextField
                ...
                placeholder="Optional purpose of this workflow..."
          ...
          <Box sx={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
            <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
              <Stack spacing={1}>
                ...
                <Typography variant="body2" color="text.secondary">
                  Define the sequential steps and responsible parties.
                </Typography>
              </Stack>
              <Button 
                variant="contained" 
        ...
        {activeStep === 2 && (
          <Box sx={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
            <Stack spacing={1}>
              ...
              <Typography variant="body2" color="text.secondary">
                Control the template activation and versioning.
              </Typography>
            </Stack>
            ...
            <Stack spacing={3}>
              <Paper variant="outlined" sx={{ p: 3, borderRadius: 1.5, display: 'flex', justifyContent: 'space-between', alignItems: 'center', bgcolor: isDark ? 'rgba(255,255,255,0.02)' : '#fafafa' }}>
                <Box>
                  <Typography variant="body2" sx={{ fontWeight: 800 }}>Initial Version</Typography>
                  <Typography variant="caption" color="text.secondary">Default version for new templates</Typography>
                </Box>
              ...
              <Paper variant="outlined" sx={{ p: 3, borderRadius: 1.5 }}>
                <FormControlLabel
                  ...
                  label={
                    <Box sx={{ ml: 1 }}>
                      <Typography variant="body2" sx={{ fontWeight: 800 }}>Enable Workflow Immediately</Typography>
                      <Typography variant="caption" color="text.secondary" sx={{ display: 'block' }}>
                        {f.pm_isactive 
                          ? 'This workflow will be active and visible for new records.' 
                          : 'This workflow will be created in an inactive (Draft) state.'}
                      </Typography>
                    </Box>
                  }
        ...
        {activeStep === 3 && (
          <Box sx={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
            <Stack spacing={1}>
              ...
              <Typography variant="body2" color="text.secondary">
                Review all settings and the approval sequence before final commitment.
              </Typography>
            </Stack>
                    ...
                    <Stack spacing={2.5}>
                      ...
                      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                        ...
                        <Box sx={{ textAlign: 'right' }}>
                          <Typography variant="caption" color="text.secondary" sx={{ display: 'block', fontWeight: 600 }}>VERSION</Typography>
                          <Typography variant="body2" sx={{ fontWeight: 800, color: 'info.main' }}>1.0.0</Typography>
                        </Box>
                      </Box>
                    </Stack>
                  </Paper>
                ...
                <Paper variant="outlined" sx={{ p: 3, borderRadius: 1.5, height: '100%' }}>
                  <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 3 }}>
                    <Typography variant="caption" sx={{ fontWeight: 900, textTransform: 'uppercase', letterSpacing: 1.5, color: 'text.disabled', display: 'block' }}>
                      Approval Sequence ({stepTemplates.length})
                    </Typography>
                  </Box>
      ...
      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        ...
        <Stack component="div" direction="row" spacing={2} sx={{ alignItems: 'center' }}>
          {activeStep < STEPS.length - 1 ? (
            ...
            >
              {saving ? 'Creating...' : 'Finalize & Create'}
            </Button>
          )}
        </Stack>
      </Box>
      ...
      <Dialog open={showStepForm} onClose={() => setShowStepForm(false)} maxWidth="sm" fullWidth slotProps={{ paper: { sx: { borderRadius: 1.5 } } }}>
        ...
        <DialogContent sx={{ pt: 1 }}>
          <Stack spacing={2.5} sx={{ mt: 1 }}>              <TextField label="Step Name" fullWidth size="small" value={stepFormData.pm_workflowname} onChange={(e) => setStepFormData(p => ({ ...p, pm_workflowname: e.target.value }))} placeholder="e.g. Finance Review" />
