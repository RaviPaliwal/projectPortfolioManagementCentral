...
export const PortfolioApprovalTaskModal: React.FC<PortfolioApprovalTaskModalProps> = ({
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      if (linkedInit && linkedInit.pm_estimatedcost != null) {
        ...
        setAskedBudget(found.pm_approvedbudgeteur ?? null)
      }
  ...
  const saveTaskData = useCallback(async (workflowDecision: number): Promise<boolean> => {
    ...
    setSaving(true)
    try {
      // DecisionBox values: 0 = Approve, 3 = Reject. Keep as Under Approval (1) if approved.
      const targetStatus = workflowDecision === 0 ? 1 : 2
