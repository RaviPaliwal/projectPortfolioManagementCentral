...
> {
  try {
    ...
    if (!resourcesResult.success) {
      console.error('[ChartService] fetchCapacityAllocationData resources failed:', resourcesResult.error)
      return []
    }
    if (!allocationsResult.success) {
      console.error('[ChartService] fetchCapacityAllocationData allocations failed:', allocationsResult.error)
      return []
    }
    ...
    const resolveResourceName = (a: Pm_resourceallocations): string | undefined => {
      ...
      const rawRes = a as unknown as Record<string, unknown>
      const normalizedGuid = normalizeLookupId((rawRes._pm_resource_value as string) || a.pm_resourceid)
    ...
    console.error('[ChartService] fetchCapacityAllocationData exception:', err)
    return []
  }
}
...
> {
  try {
    ...
    if (!allocationsResult.success) {
      console.error('[ChartService] fetchPlannedVsActualData allocations failed:', allocationsResult.error)
      return []
    }
    if (!entriesResult.success) {
      console.error('[ChartService] fetchPlannedVsActualData entries failed:', entriesResult.error)
      return []
    }
    ...
    console.error('[ChartService] fetchPlannedVsActualData exception:', err)
    return []
  }
}
...
> {
  try {
    ...
    if (!entriesResult.success) {
      console.error('[ChartService] fetchUtilizationByProjectData entries failed:', entriesResult.error)
      return []
    }
    ...
    const projectIds = Array.from(new Set(entries
      .map((entry) => normalizeLookupId((entry as unknown as Record<string, unknown>)._pm_project_value as string))
    ...
    if (projectIds.length > 0) {
      ...
      if (projectsResult.success) {
        ...
        console.error('[ChartService] fetchUtilizationByProjectData projects failed:', projectsResult.error)
      }
    }
    ...
    for (const e of entries) {
      const projectId = normalizeLookupId((e as unknown as Record<string, unknown>)._pm_project_value as string)
    ...
    console.error('[ChartService] fetchUtilizationByProjectData exception:', err)
    return []
  }
}
...
> {
  try {
    ...
    if (!resourcesResult.success) {
      console.error('[ChartService] fetchDepartmentDemandData resources failed:', resourcesResult.error)
      return []
    }
    if (!allocationsResult.success) {
      console.error('[ChartService] fetchDepartmentDemandData allocations failed:', allocationsResult.error)
      return []
    }
    ...
    const resolveDept = (a: Pm_resourceallocations): string => {
      ...
      const rawAlloc = a as unknown as Record<string, unknown>
      const normalizedGuid = normalizeLookupId((rawAlloc._pm_resource_value as string) || a.pm_resourceid)
    ...
    console.error('[ChartService] fetchDepartmentDemandData exception:', err)
    return []
  }
}
...
> {
  try {
    ...
    if (!result.success) {
      console.error('[ChartService] fetchPortfolioTrendData failed:', result.error)
      return []
    }
    ...
    console.error('[ChartService] fetchPortfolioTrendData exception:', err)
    return []
  }
}
